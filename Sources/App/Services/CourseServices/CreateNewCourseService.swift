...
struct CreateNewCourseService {
    ...
    static func createNewCourse(_ createCourse: NewCourse, _ req: Request) throws -> EventLoopFuture<CourseModel> {
        let user = try req.auth.require(UserModel.self)
        let instructorName = user.name
        let instructorLastName = user.lastname
        let fullName = "\(String(describing: instructorName)) \(String(describing: instructorLastName))"
